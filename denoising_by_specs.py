...
import subprocess
import os
if os.path.exists('./tmp/video.wav'):
    os.remove('./tmp/video.wav')

    # video to audio
subprocess.run(
    'ffmpeg -i "./tmp/video".mp4 "./tmp/video".wav', shell=True)
...
def prediction(weights_path, name_model, audio_dir_prediction, dir_save_prediction, audio_input_prediction,
    ...
    # print(audio)
    #  yeh code audio kay end me cut ko fix kar rha hai. inshallah
    # audio = audio.flatten()
    # lenght_noisy = len(audio)

    # print(lenght_noisy, "lenght noisy")
    # while lenght_noisy % 8064 != 0:
    #     lenght_noisy = lenght_noisy + 1
    # print(lenght_noisy)
    # needed_padding_zero = lenght_noisy - len(audio)
    # print(needed_padding_zero)
    # zeros = [0] * needed_padding_zero
    # # if needed_padding_zero == 0:
    # #     zeros = [0] * 100

    # print(len(zeros))
    # audio = np.concatenate((audio, zeros), axis=0)
    # print(len(audio))
    # iskay oper jo code hai woh audio kay end me jo cut araha hai usko fix kar raha hai
...
audio_dir_prediction = r'./tmp'
dir_save_prediction = r'./tmp/'
audio_input_for_pred = ['video.wav']
...
if os.path.exists('./tmp/muted_video.mp4'):
    os.remove('./tmp/muted_video.mp4')

if os.path.exists('E:/toil/FYP/client/src/denoised-video/denoised.mp4'):
    os.remove('E:/toil/FYP/client/src/denoised-video/denoised.mp4')

# muting the video
command_to_mute_video = 'ffmpeg -i ./tmp/video.mp4 -c copy -an ./tmp/muted_video.mp4'
subprocess.call(command_to_mute_video, shell=True)

# audio boosted manual work by danish.
# import required library
wav_file = pydub.AudioSegment.from_file(
    file="./tmp/denoised.wav", format="wav")
...
# Feel the difference!
new_wav_file.export(
    out_f="./tmp/denoised.wav", format="wav")

# # audio boosting work , pysndfx nahi chal raha.
...
# infile = r'FYP/server/tmp/denoised.wav'
# # infile = 'denoised_from_dataset.wav'
# outfile = r'FYP/server/tmp/denoised.wav'
...
# combining the denoised audio and muted video
command_to_combine_aud_vid = 'ffmpeg -i "./tmp/muted_video.mp4" -i "./tmp/denoised.wav" -shortest ../client/src/denoised-video/denoised.mp4'
subprocess.call(command_to_combine_aud_vid, shell=True)

#  spectrogram generation for noisy input and denoised output

print("making spectrograms")
noisy_path = r'./tmp/video.wav'
denoised_path = r'./tmp/denoised.wav'
...
fig.savefig(r'../client/src/spectograms/spec_noisy.png')
...
fig.savefig(r'../client/src/spectograms/spec_denoised.png')
